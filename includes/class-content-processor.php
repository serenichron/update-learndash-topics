...
class TSTPrep_CC_Content_Processor {
    ...
    private static function divi_to_html_cleanup($content) {
        // Remove outer Divi section and row
        $content = preg_replace('/\[et_pb_section.*?\](.*)\[\/et_pb_section\]/s', '$1', $content);
        $content = preg_replace('/\[et_pb_row.*?\](.*)\[\/et_pb_row\]/s', '$1', $content);
    
        // Convert Divi columns to div elements
